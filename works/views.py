...
def challan_no(request):
    ...
    return render(request, 'challan.html',
                  {'challan_number': challan_number, 'melt_challan_number': melt_challan_number})
...
def report_assembly(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return excel_export(report, 'Assembly_Report_' + calendar.month_name[
                int(request.POST.get('month'))] + '_' + request.POST.get('year'))
    else:
...
def excel_export_melt(reports, filename):
    ...
    sheet.merge_range(row, 0, row + 2, 5,
                      'Receives the above mentioned good in good working condition \n\n\nSend Through \t \t \tReceived By',
                      data)
...
def report_melt(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return excel_export_melt(report, 'Melt_Report_' + calendar.month_name[
                int(request.POST.get('month'))] + '_' + request.POST.get('year'))
    else:
        form = MeltReportForm()
...
def stock_report_monthly(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            report = MeltReport.objects.filter(query)
            return stock_report(report, 'Stock_Report_' + calendar.month_name[
                int(request.POST.get('month'))] + '_' + request.POST.get('year'),
                                calendar.month_name[int(request.POST.get('month'))], request.POST.get('year'))
    else:
